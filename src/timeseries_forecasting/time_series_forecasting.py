...
class RNNModel:
    ...
    def construct_network(self, inputs, outputs, layers, units=None):
        ...
        for i in range(layers):
            print(i)
            # First LSTM layer
            x = Bidirectional(LSTM(units[i], activation='relu', return_sequences=(i < layers - 1)))(x)
    ...
    def calculate_mean_deviation(self, x_test, y_test, output_length):
        angle_1_deviation = []
        angle_2_deviation = []
        for i, (x_sample, y_sample) in enumerate(zip(x_test, y_test)):
            ...
            y_pred = self.model.predict(np.expand_dims(x_sample, axis=0)).reshape(-1, 4)
            y_sample = y_sample.reshape(-1, 4)
            angle_1_groundtruth_1 = y_sample[:, 0]
            angle_1_groundtruth_2 = y_sample[:, 1]
            angle_1_groundtruth = np.mean(np.stack((angle_1_groundtruth_1, angle_1_groundtruth_2), axis=0), axis=0)

            angle_1_prediction_1 = y_pred[:, 0]
            angle_1_prediction_2 = y_pred[:, 1]
            angle_1_prediction = np.mean(np.stack((angle_1_prediction_1, angle_1_prediction_2), axis=0), axis=0)

            angle_2_groundtruth_1 = y_sample[:, 2]
            angle_2_groundtruth_2 = y_sample[:, 3]
            angle_2_groundtruth = np.mean(np.stack((angle_2_groundtruth_1, angle_2_groundtruth_2), axis=0), axis=0)

            angle_2_prediction_1 = y_pred[:, 2]
            angle_2_prediction_2 = y_pred[:, 3]
            angle_2_prediction = np.mean(np.stack((angle_2_prediction_1, angle_2_prediction_2), axis=0), axis=0)

            angle_1_deviation.append(abs(angle_1_groundtruth - angle_1_prediction))
            angle_2_deviation.append(abs(angle_2_groundtruth - angle_2_prediction))
        angle_1_mean_deviation = np.mean(angle_1_deviation, axis=0)
        angle_2_mean_deviation = np.mean(angle_2_deviation, axis=0)

        print(np.std(angle_1_mean_deviation))
        print(np.std(angle_2_mean_deviation))
        print(np.mean(angle_1_mean_deviation))
        print(np.mean(angle_2_mean_deviation))

        # Get the indices for the plot
        indices = range(output_length)

        # Plot both Angle1 and Angle2 deviation on the same graph
        plt.plot(indices, angle_1_mean_deviation, label='Angle1 Deviation')
        plt.plot(indices, angle_2_mean_deviation, label='Angle2 Deviation')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-bs', '--batch_size', default=520, type=int, help="", required=False)
    ...
    parser.add_argument('-u', '--units', default=[128, 256], type=list, help="", required=False)
    ...
    if args.evaluate:
        model.calculate_mean_deviation(x_test, y_test, args.output_length)
